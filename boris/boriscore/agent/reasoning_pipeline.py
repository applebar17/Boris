...
from boriscore.ai_clients.ai_clients import ClientOAI
from boriscore.code_structurer.code_manager import CodeProject, ProjectNode
from boriscore.agent.prompts import (
    REASONING_STEP_1_V2,
    AGENT_CHAT_MESSAGE,
    AGENT_SYSTEM_PROMPT,
    AGENT_CHAT_MESSAGE_JIRA,
)
from boriscore.utils.utils import handle_path, log_msg
from boriscore.models.api import JiraProject, JiraStory
from boriscore.models.ai import ReasoningAI, ReasoningAIAgentVersion
...
class CodeWriter(CodeProject, ProjectNode):
    ...
    def __init__(
        ...
        logger: logging = None,
        base_path: Path = Path("."),
        code_writer_toolbox_path: Path = Path("app/backend/agent/toolbox.json"),
        ...
        **kwargs,
        ...
        self.code_writer_toolbox_path = self.base_path / code_writer_toolbox_path
        self._log(f"code_writer_toolbox path = {self.code_writer_toolbox_path}")
        ...
        with open(self.code_writer_toolbox_path, mode="r") as fp:
            self.code_writer_toolbox: dict = json.load(fp=fp)
    ...
    def update_tool_mapping(self, original_request: str = None) -> None:
        if self.code_writer_toolbox_path.parts[-1].__str__() == "toolbox.json":
            # V1 toolbox: (pure functions) (toolbox.json)
            self.code_writer_tools_mapping = {
                ...
                # "run_bash_command": self.run_bash,
                # "initialize_react_project": self.initialize_react_project,
            }
        elif self.code_writer_toolbox_path.parts[-1].__str__() == "toolbox_v2.json":
            # V2 toolbox: Agentic-based code generation functions (toolbox_v2.json)
            self.code_writer_tools_mapping = {
                "retrieve_node": self.retrieve_node,
                "create_node": partial(
            ...
            raise ValueError(
                f"{self.code_writer_toolbox_path.parts[-1].__str__()} isn't a mapped toolbox at the moment. Allowed toolboxes: toolbox.json, toobox_v2.json"
            )
    ...
    def reasoning_step_jira(
        ...
        params = self.handle_params(
            ...
            model=self.reasoning_model if self.reasoning_model else self.llm_model,
            temperature=None,
