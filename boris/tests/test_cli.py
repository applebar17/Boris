...
def test_chat_script_mode_runs_and_exits(temp_project: Path, monkeypatch):
...
def test_version_command():
    ...
    # should print a version like "0.1.0"
...
def test_chat_help():
    ...
    assert result.exit_code == 0
    assert "Start terminal chat" in result.stdout


def test_chat_script_mode_runs_and_exits(tmp_path, monkeypatch):
    # run chat in script mode (non-interactive)
    result = runner.invoke(
        app, ["chat", "--script", "hello;/exit"], catch_exceptions=False
    )
    assert result.exit_code == 0
    # should contain "boris" panel text or the assistant’s placeholder answer
    assert "boris" in result.stdout.lower() or "answer" in result.stdout.lower()
