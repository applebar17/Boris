...
def test_echo_command(terminal_executor):
    # Test safe command execution
    result = terminal_executor.run_terminal_tool("echo hello")
    assert result.returncode == 0
    assert "hello" in result.stdout


def test_denylist_command(terminal_executor):
    # Test a command that should be denied
    result = terminal_executor.run_terminal_tool("rm -rf /")
...
def test_safe_command_execution(terminal_executor):
    # Test another safe command execution
    result = terminal_executor.run_terminal_tool("ls")
    assert result.returncode == 0
    assert "some_file.txt" in result.stdout  # Assuming some_file.txt exists in /tmp


def test_denylist_error_handling(terminal_executor):
    # Test a command that should be denied
